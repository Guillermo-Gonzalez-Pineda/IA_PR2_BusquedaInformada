...
void Busqueda_A::inicializarBusqueda(std::ofstream& archivo_salida, int indice_ejecucion) {
  unsigned seed = static_cast<unsigned>(time(nullptr)) + indice_ejecucion;
  srand(seed);
  ...
  //Repetimos el bucle 10 veces
  int aux = 0;
  while(!nodos_abiertos_.empty()) {
    Nodo* nodo_actual;
    //En la segunda iteracion, en lugar de añadir el nodo con menor coste heurístico, añadimos un nodo aleatorio
    if(aux == 1) {    //Si nos encontramos en la segunda iteración
      //Incrementamos el valor de aux para no repetir esta condición
      aux++;        
      //Hacemos que nuestro nodo actual sea un nodo aleatorio del vector de nodos abiertos
      int indice_nodo_aleatorio = rand() % nodos_abiertos_.size();
      nodo_actual = nodos_abiertos_[indice_nodo_aleatorio];

      //Eliminamos el nodo actual de nodos_abiertos_
      nodos_abiertos_.erase(nodos_abiertos_.begin() + indice_nodo_aleatorio);
      ...
      nodo_actual = nodos_abiertos_[indice_menor_coste_heuristico];
      //Eliminamos el nodo actual de nodos_abiertos_
